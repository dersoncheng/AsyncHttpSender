...
public abstract class MyAsyncTask<Params, Progress, Result> {
	private static final String LOG_TAG = "HPAsyncTask";
	...
	private static final ThreadFactory sThreadFactory = new ThreadFactory() {
		...
		public Thread newThread(Runnable r) {
			Thread tread = new Thread(r, "HPAsyncTask #"
	...
	private static final int CORE_POOL_SIZE_LONG = 0;
	private static final int MAXIMUM_POOL_SIZE_LONG = CORE_POOL_SIZE;
	private static final int KEEP_ALIVE_LONG = 1;
	private static final BlockingQueue<Runnable> sPoolWorkQueue_LONG = new LinkedBlockingQueue<Runnable>();

	private static final ThreadFactory sThreadFactory_LONG = new ThreadFactory() {
		private final AtomicInteger mCount = new AtomicInteger(1);

		public Thread newThread(Runnable r) {
			Thread tread = new Thread(r, "HPAsyncTask_LONG #"
					+ mCount.getAndIncrement());
			tread.setPriority(Thread.NORM_PRIORITY - 1);
			return tread;
		}
	...
	public static final ThreadPoolExecutor THREAD_POOL_LONG = new ThreadPoolExecutor(
			CORE_POOL_SIZE_LONG, MAXIMUM_POOL_SIZE_LONG, KEEP_ALIVE_LONG,
			TimeUnit.MINUTES, sPoolWorkQueue_LONG, sThreadFactory_LONG);
	...
	private Result postResult(Result result) {
		...
		Message message = sHandler.obtainMessage(MESSAGE_POST_RESULT,
				new HupuAsyncTaskResult<Result>(this, result));
	...
	protected final void publishProgress(Progress... values) {
		if (!isCancelled()) {
			sHandler.obtainMessage(MESSAGE_POST_PROGRESS,
					new HupuAsyncTaskResult<Progress>(this, values))
					.sendToTarget();
		}
	}
	...
	private static class InternalHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			HupuAsyncTaskResult result = (HupuAsyncTaskResult) msg.obj;
			switch (msg.what) {
	...
	private static class HupuAsyncTaskResult<Data> {
		final MyAsyncTask mTask;
		final Data[] mData;

		HupuAsyncTaskResult(MyAsyncTask task, Data... data) {
			mTask = task;
			mData = data;
		}
	}

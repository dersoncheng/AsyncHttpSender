...
import com.hupu.android.net.AsyncHttpClient.toolbox.HPHttpError;
import com.hupu.android.net.AsyncHttpClient.toolbox.HPTimeoutError;
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    public void setUseSynchronousMode(boolean sync) {
        ...
        if (!sync && looper == null) {
            sync = true;
            HPLog.w(LOG_TAG, "Current thread has not called Looper.prepare(). Forcing synchronous mode.");
        }
     ...
     */
    public void onProgress(int bytesWritten, int totalSize) {
    	//HPLog.v(LOG_TAG, String.format("Progress %d from %d (%2.0f%%)", bytesWritten, totalSize, (totalSize > 0) ? (bytesWritten * 1.0 / totalSize) * 100 : -1));
    }
     ...
     */
    public void onRetry(int retryNo) {
    	HPLog.d(LOG_TAG, String.format("Request retry no. %d", retryNo));
    }

    public void onCancel() {
    	HPLog.d(LOG_TAG, "Request got cancelled");
    }
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if (response != null && response.length >= 3) {
                	...
                	HPLog.e(LOG_TAG, "SUCCESS_MESSAGE didn't got enough params");
                }
                break;
            case FAILURE_MESSAGE:
                ...
                if (response != null && response.length >= 4) {
                	...
                	HPLog.e(LOG_TAG, "FAILURE_MESSAGE didn't got enough params");
                }
                break;
            case START_MESSAGE:
                onStart();
            ...
            case PROGRESS_MESSAGE:
                ...
                if (response != null && response.length >= 2) {
                    try {
                    	...
                    	HPLog.e(LOG_TAG, "custom onProgress contains an error", t);
                    }
                } else {
                	HPLog.e(LOG_TAG, "PROGRESS_MESSAGE didn't got enough params");
                }
                break;
            case RETRY_MESSAGE:
                response = (Object[]) message.obj;
                if (response != null && response.length == 1) {
                	...
                	HPLog.e(LOG_TAG, "RETRY_MESSAGE didn't get enough params");
                }
                break;
            case CANCEL_MESSAGE:
                onCancel();
        ...
        if (!Thread.currentThread().isInterrupted()) {
            ...
            if (!Thread.currentThread().isInterrupted()) {
                if (status.getStatusCode() >= 300) {
                    HPHttpError error = new HPTimeoutError("连接超时,请重试");
                    sendFailureMessage(status.getStatusCode(), response.getAllHeaders(), responseBody, error);
